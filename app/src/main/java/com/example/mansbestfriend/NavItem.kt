...
fun NavGraph(navController: NavHostController) {
    NavHost(navController, startDestination = Screen.Home.route) {
        composable(Screen.Home.route) { BreedScreen(nav =navController) }
        composable("p4/{character_id}") { parameter ->
            val breed_id: String? = parameter.arguments?.getString("character_id")
            BreedScreen(breed_id = breed_id!!,nav = navController)
        }
